...
public interface GymMapper {
    ...
    @Insert("insert into TLG_City(ProvinceId,CH_Name,EN_Name,Sort,Status)select id,#{city},[dbo].[fn_GetPinyin](replace(#{city},'市','')),0,2 from TLG_Province p join (select 1 x)x on p.CH_Name like '${prov}%' left join TLG_City c on c.ProvinceId=p.id and charindex(c.CH_Name,#{city})>0 where c.M_Id is null;" +
